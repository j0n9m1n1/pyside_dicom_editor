...
class MainWindow(QMainWindow):
    ...
    def ui_setup(self):
        ...
        # 호기심 해결겸 list comp
        [
            self.grid_layout.addWidget(self.label_tags[i], i + 1, 1)
            for i in range(count_list_tag)
        ]
        [
            self.grid_layout.addWidget(self.line_edit_tags[i], i + 1, 2)
            for i in range(count_list_tag)
        ]

        self.grid_layout.addWidget(
            self.table_widget, 1, 0, count_list_tag + 1, 1
        )
        ...
        for i, item in enumerate(list_selected_index):
            for j, tag in enumerate(list_tag):
                if self.line_edit_tags[j].text() != "":
                    ...
                    # 생각해보니 파일명을 가지고 있지 않음, output 경로도 넣어야함
                    # output 경로는 id_name_studyuid\n 정도?
    ...
    def button_clicked_clear(self):
        [self.line_edit_tags[i].setText("") for i in range(len(list_tag))]
    ...
    def insert_file_list_to_table_widget(self, path):
        ...
        for i, file_name in enumerate(dict_files):
            ...
            list_ds.append(
                dcmread(file_path, stop_before_pixels=True, force=True)
            )
            # print(ds)
            for j, tag in enumerate(list_tag):
